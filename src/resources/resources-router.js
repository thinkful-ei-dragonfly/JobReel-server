...
resourcesRouter
  ...
  .patch('/:resource_id', jsonParser, (req, res, next) => {
    const { resource_id } = req.params
    const { type, title, description, date_added, } = req.body
    const updatedResource = { type, title, description, date_added }

    const numberOfValues = Object.values(updatedResource).filter(Boolean).length
    if (numberOfValues === 0) {
      return res
        .status(400)
        .json({
